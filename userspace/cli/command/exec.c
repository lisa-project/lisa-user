...
int build_config_interface(struct cli_context *ctx, FILE *out, struct net_switch_dev *nsdev)
{
	int sock_fd;

	SW_SOCK_OPEN(ctx, sock_fd);

	fprintf(out, "if %d %d\n", nsdev->ifindex, nsdev->type);
...
int build_config_global(struct cli_context *ctx, FILE *out, int sock_fd)
{
	...
	for (i = 0; i < if_map.size; i++) 
		build_config_interface(ctx, out, &if_map.dev[i]);
...
static __inline__ int __cmd_show_run(struct cli_context *ctx, int argc, char **argv, struct menu_node **nodev)
{
	...
	ret = nsdev.ifindex ? build_config_interface(ctx, tmp_out, &nsdev) :
