...
static DECLARE_MUTEX(sw_ioctl_mutex);
static int (*sw_ioctl_hook)(unsigned int, void __user *) = NULL;

void swioctl_set(int (*hook)(unsigned int, void __user *)) {
	down(&sw_ioctl_mutex);
	sw_ioctl_hook = hook;
	up(&sw_ioctl_mutex);
}
EXPORT_SYMBOL(swioctl_set);
